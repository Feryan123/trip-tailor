...
import { Menu, Plus, MessageSquare, X, Loader2, ImageIcon, Trash2 } from 'lucide-react';
...
export default function ChatPage({ params }: ChatPageProps) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Create the message payload with proper typing
      const messagePayload: SendMessageWithImages = {
        message: currentInput,
        conversationId: conversationId || id,
        images: imageBase64Array.length > 0 ? imageBase64Array : undefined
      };

      // If your API doesn't support images yet, you can send without images
      const data = await chatAPI.sendMessage({
        message: currentInput,
        conversationId: conversationId || id,
        // Remove or comment out the images line if your API doesn't support it yet
        ...(imageBase64Array.length > 0 && { images: imageBase64Array })
      } as any); // Type assertion as temporary fix until API types are updated
